...
export default Vue.extend({
  data() {
    return {
      redDice: new Dice(20, 15, 100, 'red'),
  ...
  mounted() {
    ...
    this.yellowDice = new Dice(
      this.redDice.length * (1 + 1 / 3),
      15,
      this.redDice.length,
      'yellow'
    )
    this.valueDisplay = new TextDisplay(
      this.yellowDice.x + this.yellowDice.length * 1,
      20,
      ''
    )
    ...
    this.diceCanvas.canvas.style.width ='100%';
  ...
  methods: {
    ...
    displayText(text: string): void {
      ...
      this.diceCanvas.fillText(
        text,
        this.valueDisplay.x + this.redDice.length / 6,
        this.valueDisplay.y + (this.redDice.length * 5) / 6
      )
      this.diceCanvas.closePath()
    },
    animateDice(): void {
      ...
      var myTimer = setInterval(() => {
        ...
        this.displayDiceValue(this.yellowDice)
        this.displayText(
          (this.redDice.value + this.yellowDice.value).toString()
        )
        counter++
        if (counter > 10) {
    ...
    displayDiceValue(dice: Dice): void {
      ...
      switch (dice.value) {
        case 1: {
          this.diceCanvas.beginPath()
          this.diceCanvas.arc(
            dice.x + dice.length * (2 / 3 - 1 / 6),
            dice.y + dice.length * (2 / 3 - 1 / 6),
            dice.length / 4,
            0,
            2 * Math.PI
          )
        ...
        case 2: {
          this.diceCanvas.beginPath()
          this.diceCanvas.arc(
            dice.x + dice.length * (1 / 3 - 1 / 12),
            dice.y + dice.length * (1 / 3 - 1 / 12),
            dice.length / 6,
            0,
            2 * Math.PI
          )
          ...
          this.diceCanvas.beginPath()
          this.diceCanvas.arc(
            dice.x + dice.length * (1 - 1 / 12 - 1 / 6),
            dice.y + dice.length * (1 - 1 / 12 - 1 / 6),
            dice.length / 6,
            0,
            2 * Math.PI
          )
        ...
        case 3: {
          this.diceCanvas.beginPath()
          this.diceCanvas.arc(
            dice.x + dice.length * (1 / 3 - 1 / 8),
            dice.y + dice.length * (1 / 3 - 1 / 8),
            dice.length / 6,
            0,
            2 * Math.PI
          )
          ...
          this.diceCanvas.beginPath()
          this.diceCanvas.arc(
            dice.x + dice.length * (1 - 1 / 24 - 1 / 6),
            dice.y + dice.length * (1 - 1 / 24 - 1 / 6),
            dice.length / 6,
            0,
            2 * Math.PI
          )
        ...
        case 6: {
          this.diceCanvas.beginPath()
          this.diceCanvas.arc(
            dice.x + dice.length * (1 / 3 - 1 / 6),
            dice.y + dice.length * (1 / 3 - 1 / 6),
            dice.length / 7.5,
            0,
            2 * Math.PI
          )
